"""Test script for Nutrient Processor parser"""
import json
import sys
...
from parsers.refinery import parse_nutrient_processor

def main():
    print("=" * 60)
    print("Testing Nutrient Processor Parser")
    print("=" * 60)
    ...
    try:
        # Parse the MXML file for cooking recipes
        recipes = parse_nutrient_processor(str(mxml_path))

        # Save to JSON
        output_path = Path(__file__).parent.parent / 'data' / 'json' / 'NutrientProcessor.json'
        with open(output_path, 'w', encoding='utf-8') as f:
            json.dump(recipes, f, indent='\t', ensure_ascii=False)

        print(f"[OK] Saved {len(recipes)} cooking recipes to {output_path.name}")
