"""Test script for Buildings parser"""
import json
import sys
...
def main():
    print("=" * 60)
    print("Testing Buildings Parser")
    print("=" * 60)

    mxml_path = Path(__file__).parent.parent / 'data' / 'mbin' / 'basebuildingobjectstable.MXML'
    ...
    try:
        items = parse_buildings(str(mxml_path))

        output_path = Path(__file__).parent.parent / 'data' / 'json' / 'Buildings.json'
        with open(output_path, 'w', encoding='utf-8') as f:
            json.dump(items, f, indent='\t', ensure_ascii=False)

        print(f"[OK] Saved {len(items)} buildings to {output_path.name}")
