"""Test script for Cooking parser"""
import json
import sys
...
def main():
    print("=" * 60)
    print("Testing Cooking Parser")
    print("=" * 60)

    mxml_path = Path(__file__).parent.parent / 'data' / 'mbin' / 'consumableitemtable.MXML'
    ...
    try:
        items = parse_cooking(str(mxml_path))

        output_path = Path(__file__).parent.parent / 'data' / 'json' / 'Cooking.json'
        with open(output_path, 'w', encoding='utf-8') as f:
            json.dump(items, f, indent='\t', ensure_ascii=False)

        print(f"[OK] Saved {len(items)} cooking items to {output_path.name}")
