"""Test script for Technology parser"""
import json
import sys
...
def main():
    print("=" * 60)
    print("Testing Technology Parser")
    print("=" * 60)

    mxml_path = Path(__file__).parent.parent / 'data' / 'mbin' / 'nms_reality_gctechnologytable.MXML'
    ...
    try:
        items = parse_technology(str(mxml_path))

        output_path = Path(__file__).parent.parent / 'data' / 'json' / 'Technology.json'
        with open(output_path, 'w', encoding='utf-8') as f:
            json.dump(items, f, indent='\t', ensure_ascii=False)

        print(f"[OK] Saved {len(items)} technologies to {output_path.name}")
